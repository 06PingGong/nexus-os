...
const ScholarHub = () => {
  ...
  // 模拟快速翻译：将常用中文关键词转为英文以获得更好的 ArXiv 结果
  ...
  const fetchPapers = async (query: string) => {
    ...
    try {
      // 使用 ArXiv API
      const response = await fetch(`https://export.arxiv.org/api/query?search_query=all:${encodeURIComponent(engQuery)}&start=0&max_results=10`);
  ...
  return (
    <div className="scholar-hub">
      ...
      <div className="hub-content">
        <aside className="hub-sidebar">
          <div className="intelligence-card glass-card">
            <h3><Zap size={14} /> 智能检索</h3>
            <p>系统已自动将 <b>{activeDomain}</b> 优化为英文指令以检索全球数据库。</p>
      ...
      <style jsx>{`
        ...
        .search-bar { display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem 1rem; margin-bottom: 2rem; border: 1px solid #e4e4e7; }
        .search-bar input { flex: 1; background: transparent; border: none; outline: none; font-size: 0.95rem; }
        .loading-state { display: flex; flex-direction: column; align-items: center; padding: 5rem; color: #a1a1aa; gap: 1rem; }
        .animate-spin { animation: spin 1s linear infinite; }
        @keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        .paper-card { padding: 1.5rem; margin-bottom: 1rem; display: flex; justify-content: space-between; align-items: center; }
        .paper-tag { display: inline-block; background: #eff6ff; color: #2563eb; padding: 0.15rem 0.5rem; border-radius: 4px; font-size: 0.7rem; font-weight: 600; margin-bottom: 0.5rem; }
        .paper-card h3 { font-size: 1.1rem; font-weight: 600; margin-bottom: 0.4rem; }
        .authors { color: #71717a; font-size: 0.85rem; margin-bottom: 0.75rem; }
        .pdf-link { color: #2563eb; text-decoration: none; font-size: 0.8rem; display: flex; align-items: center; gap: 0.4rem; }
        .open-link-btn { width: 44px; height: 44px; border-radius: 12px; background: #f9fafb; border: 1px solid #e4e4e7; display: flex; align-items: center; justify-content: center; cursor: pointer; }
        .api-status { font-size: 0.7rem; color: #10b981; font-weight: 600; }
      `}</style>
    </div>
  );
};
